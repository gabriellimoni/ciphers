...
import { RequiredParamError, EntityAlreadyExistsError } from '../../errors'
...
describe('AddChord Controller', () => {
  test('Shoudl return 500 if AddChordUsecase throws', async () => {
    ...
    jest.spyOn(addChordUsecaseStub, 'exec').mockReturnValueOnce(new Promise((resolve, reject) => reject(new Error())))
  ...
  test('Shoudl return 400 and ChordAlreadyExists message if AddChordUsecase returns null', async () => {
    const { sut, addChordUsecaseStub } = makeSut()
    jest.spyOn(addChordUsecaseStub, 'exec').mockReturnValueOnce(new Promise(resolve => resolve(null)))
    const response = await sut.handle({
      body: {
        symbol: 'any_symbol',
        imagesUrls: ['any_iamges_urls']
